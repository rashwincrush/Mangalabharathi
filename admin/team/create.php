...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = dirname(dirname(__DIR__)) . '/uploads/team/';
        ...
        if (!in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
                $image_url = 'uploads/team/' . $new_filename;
            } else {
    ...
    if (empty($errors)) {
        // Create team member
        $stmt = $conn->prepare("
            INSERT INTO team_members (full_name, position, bio, image_url, email, linkedin_url, display_order, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Error creating team member: " . $conn->error;
        }
    }
}

// Get max display order for suggestion
$max_order_result = $conn->query("SELECT MAX(display_order) as max_order FROM team_members");
...
<head>
    ...
    <title>Add Team Member - <?php echo SITE_NAME; ?></title>
